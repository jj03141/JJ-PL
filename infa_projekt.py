...
class transformacje:
    ...
    def file_saveNEU(self, X, Y, Z, xref, yref, zref, file_out):
        ...
        U = []
        for a, b, c in zip(X, Y, Z):
            n, e, u = geo.xyz2neu(a, b, c, xref, yref, zref)
            N.append(n)
            E.append(e)
            U.append(u)
...
if __name__ == '__main__':
    ...
    '''
    parser = arg.ArgumentParser(description = 'Transformacje')
    parser.add_argument('--input', type = str, choices = ['cmd','txt'], default = 'cmd', help = 'Wybierz czy chcesz transformować współrzędne wprowadzone ręcznie (cmd) czy z pliku tekstowego (txt), domyslnie: cmd')
